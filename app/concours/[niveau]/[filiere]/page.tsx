...
export default async function ConcoursListingPage(props: { params: Promise<{ niveau: string; filiere: string }> }) {
    ...
    return (
        <div className="container mx-auto px-4 py-12">
            <div className="mb-8">
                <Link href={`/concours/${niveau}`} className="text-sm font-medium text-muted-foreground hover:text-primary flex items-center gap-1 mb-4">
                    <ArrowLeft className="h-4 w-4" /> Retour aux filières
                </Link>
                <h1 className="text-3xl font-bold tracking-tight uppercase">
                    Concours {filiere} - <span className="text-primary">{niveau.replace("-", " ")}</span>
                </h1>
                <p className="text-muted-foreground"> {filteredExams.length} sujets disponibles</p>
            </div>
